...
static plugin_ref get_auth_plugin(THD *thd, const LEX_CSTRING &name)
{
...
static int set_user_auth(THD *thd, ACL_USER *acl_user, const LEX_CSTRING &pwtext)
{
  ...
  plugin_ref plugin= get_auth_plugin(thd, acl_user->plugin);
...
static bool acl_load(THD *thd, const Grant_tables& tables)
{
  ...
  while (!(read_record_info.read_record()))
  {
    ...
    */
    ...
    if (set_user_auth(thd, &user, empty_clex_str))
    {
      thd->clear_error(); // the warning is still issued
      continue;
    }
                        ...
                        MPVIO_EXT *mpvio)
{
  ...
  plugin_ref plugin= get_auth_plugin(thd, *auth_plugin_name);
